...
TokenType Lexer::keywordType(const std::string& ident) {
    if (ident == "let")     return TokenType::KW_LET;
    if (ident == "fn")      return TokenType::KW_FN;
    if (ident == "if")      return TokenType::KW_IF;
    if (ident == "else")    return TokenType::KW_ELSE;
    if (ident == "while")   return TokenType::KW_WHILE;
    if (ident == "return")  return TokenType::KW_RETURN;
    ...
    switch (c) {
        ...
        case '&':
            ...
            // Single & is not supported
            {
                ...
                oss << "Unexpected character '&' at line " << startLine
                    << ", column " << startCol
                    << ". Use '&&' for logical AND.";
