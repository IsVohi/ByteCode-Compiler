...
public:
    ...
    BinaryOpExpr(std::unique_ptr<Expr> left,
                 Operator op,
...
public:
    IfStmt(std::unique_ptr<Expr> condition, std::vector<std::unique_ptr<Stmt>> body)
...
/**
 * Represents function return: return expr;
 * Examples: return 1; return n * factorial(n - 1);
 */
...
/**
 * Represents the entire program: collection of function declarations and statements.
 ...
 * Each concrete visitor implements these methods to perform different operations:
 * - Code generation
 * - Type checking
 * - Optimization
 * - Pretty printing
